...
class Trainer:
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch in train_generator_tqdm:
            if batch_num > 0 and np.random.rand() > 0.0001:
                break
            batch_num += 1
            ...
            metrics = self._get_metrics(train_loss, batch_num, reset=False)
    ...
    def _validation_loss(self) -> Tuple[float, int]:
        ...
        for b in np.arange(-10.0, -0.0, 0.3):
            print('Bias now: ', b)
            self._model.bias_outside.data = torch.zeros(1) + b
            ...
            for batch in val_generator_tqdm:
                batch_num += 1
                if batch_num%(num_validation_batches//3) > 500:
                    continue
                self._optimizer.zero_grad()
                ...
                if self._model.new and batch_num > 1:
                    ...
                    print(val_metrics['f1-measure-overall'])
    ...
    def train(self) -> Dict[str, object]:
        ...
        for epoch in range(epoch_counter, self._num_epochs):
            ...
            self.f1s_train.append(train_metrics['f1-measure-overall'])
            self.losses_train.append(train_metrics['loss'])

            np.save('results/losses_train', self.losses_train)
            np.save('results/f1_train', self.f1s_train)
            if self._validation_data is not None:
                # We have a validation set, so compute all the metrics on it.
                val_loss, num_batches = self._validation_loss()
                val_metrics = self._get_metrics(val_loss, num_batches, reset=True)
                self.f1s_valid.append(val_metrics['f1-measure-overall'])
                self.losses_valid.append(val_loss[0])
                np.save('results/losses', self.losses_valid)
                np.save('results/f1', self.f1s_valid)
    ...
    def _save_checkpoint(self,
        ...
        if self._serialization_dir is not None:
            ser_dir_now = self._serialization_dir

            import subprocess
            cmd = 'cp -r ' + self._serialization_dir + ' ' + ser_dir_now
            #p = subprocess.Popen(cmd.split())
            #p.wait()

            model_path = os.path.join(ser_dir_now, "model_state_epoch_{}.th".format(epoch))
            ...
            training_state = {'epoch': epoch,
                              ...
                              'batch_num_total': self._batch_num_total}
            training_path = os.path.join(ser_dir_now,
                                         "training_state_epoch_{}.th".format(epoch))
            torch.save(training_state, training_path)
            if is_best:
                logger.info("Best validation performance so far. "
                            "Copying weights to '%s/best.th'.", ser_dir_now)
                shutil.copyfile(model_path, os.path.join(ser_dir_now, "best.th"))
