...
@DataIterator.register("maml")
class MAMLIterator(DataIterator):
    """
    ...
    def _create_batches(self, instances: Iterable[Instance], shuffle: bool) -> Iterable[Batch]:
        ...
        for instance_list in self._memory_sized_lists(instances):
            ...
            for batch_instances in lazy_groups_of(iterator, self._batch_size * 30):
    ...
    def _yield_one_epoch(self, instances: Iterable[Instance], shuffle: bool, cuda_device: int, for_training: bool):
        batches = self._create_batches(instances, False)
        import numpy as np
        i=0
        for batch in batches:
            ...
            result = batch.as_tensor_dict(padding_lengths,
                                   cuda_device=cuda_device,
                                   for_training=for_training)
            if for_training:
                for key, value in result['tokens'].items():
                    result['tokens'][key] = value.view((30, self._batch_size) + value.shape[1:])
                result['tags'] = result['tags'].view((30, self._batch_size) + result['tags'].shape[1:])
            yield result

    @classmethod
    def from_params(cls, params: Params) -> 'MAMLIterator':
