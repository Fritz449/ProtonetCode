    ...
    for i, domain in enumerate(domains):
        ...
        for n, key in enumerate(this_train):
            ...
            for item in data:
                ...
                all_sentences[-1].append(sentence)

    super_list = []
    for _ in range(n_episodes):
        sentences = random.choice(all_sentences)
        ys_sup = np.array([])
        support = []
        query = []
        for i in range(total_sentences):
            if file == 'train':
                sentence = random.choice(sentences[:(len(sentences) // 2)])
            else:
                sentence = random.choice(sentences[(len(sentences) // 2):])
            ys_here = [xy[1] for xy in sentence]
            if (ys_sup.shape[0] < np.unique(np.concatenate((ys_sup, np.unique(ys_here)))).shape[0]) or ((len(query)+1)/len(support))>target_relation:
...
@DatasetReader.register("snips_pnet")
class PnetSnipsDatasetReader(DatasetReader):
    """
    ...
    @classmethod
    def from_params(cls, params: Params) -> 'PnetSnipsDatasetReader':
        token_indexers = TokenIndexer.dict_from_params(params.pop('token_indexers', {}))
        tag_label = params.pop('tag_label', None)
        feature_labels = params.pop('feature_labels', ())
        lazy = params.pop('lazy', False)
        params.assert_empty(cls.__name__)
        return PnetSnipsDatasetReader(token_indexers=token_indexers,
                                  tag_label=tag_label,
