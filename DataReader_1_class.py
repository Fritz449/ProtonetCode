...
def snips_reader(file='train', dataset_download_path='ontonotes/', return_intent=False):
    ...
    ys, counts = np.unique([y[2:] for y in ys], return_counts=True)

    valid_classes = ['ORG', 'CARDINAL', 'MONEY']
    train_classes = [cl for cl in ys if (cl not in valid_classes and cl != '')]
    ...
    total_sentences = 100
    train_sentences = 50
    train_sentences_with_classes = []
    test_sentences_with_classes = []
    base_sentences = copy.deepcopy(sentences)
    super_list = []
    for class_here in classes_here:
        sentences = copy.deepcopy(base_sentences)
        for sent in sentences:
            for word in sent:
                if word[1][2:] != class_here:
                    word[1] = 'O'
        ...
        if file != 'train.txt':
            ...
            for sentence in sentences:
                ...
                if np.unique(ys_here).shape[0] > 1 and len(train) < train_sentences:
            ...
            train_sentences_with_classes.append([])
            test_sentences_with_classes.append([])
            nonempty = []
            empty = []
            for sentence in sentences:
                ys_here = [xy[1] for xy in sentence]
                if np.unique(ys_here).shape[0] > 1:
                    nonempty.append(sentence)
                else:
                    empty.append(sentence)
            tmp = nonempty + np.random.choice(empty, size=len(nonempty)//2).tolist()
            train_sentences_with_classes[-1] += tmp[::2]
    ...
    if file == 'train.txt':
        ...
        for _ in range(n_episodes):
            ...
            query_sentences = np.random.choice(test_sentences_with_classes[i_class], total_sentences-train_sentences, False).tolist()
            super_list += support_sentences + query_sentences
    return super_list
